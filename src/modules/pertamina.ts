...
export class Pertamina {
  ...
  private linkTos =
    "https://subsiditepatlpg.mypertamina.id/merchant/app/onboarding/terms-conditions";
  ...
  private linkCheckNIK =
    "https://api-map.my-pertamina.id/customers/v2/verify-nik?nationalityId=";
  private linkTransaction =
    "https://api-map.my-pertamina.id/general/v1/transactions";
  private linkTransactionV2 =
    "https://api-map.my-pertamina.id/general/v2/transactions";
  ...
  async login() {
    ...
    try {
      const userForm = page.getByPlaceholder(
      ...
      console.error(e);
      await this.bot.sendPhotoToAdmin(
    ...
    if (message.length > 800) {
      ...
      if (page.url() == this.linkTos) {
        ...
        await page.click(
          "#mantine-r4-body > div:nth-child(3) > div:nth-child(1) > button:nth-child(1)",
        );
      ...
      await this.bot.sendPhotoToAdmin(
        await page.screenshot(),
        "[-] Error login: no authorization headers",
      );
    }
  ...
  async getReport(date: string) {
    try {
      const req = await got(
        `${this.linkReport}?startDate=${date}&endDate=${date}`,
        {
          ...this.options,
        },
      );
      ...
      for (const date of [now, yesterday]) {
        ...
        if (report && report.success) {
          for (const customerReport of report.data.customersReport) {
            if (
              customerReport.total > this.stockMap[customerReport.categories[0]]
            ) {
              try {
                const req = await got(
                ...
                if (req.statusCode == 200) {
                  ...
                  if (transactionReport.success) {
                    for (
                      let i = 1;
                      i < transactionReport.data.data.length;
                      i++
                    ) {
                      ...
                      if (!transaction.isCanceled) {
                        try {
                          const req = await got(
                            `${this.linkTransaction}/${transaction.transactionId}/cancel`,
                            {
                              method: "post",
                              body: JSON.stringify({
                                reason: "Salah",
                          ...
                          if (JSON.parse(req.body).success) {
                            messages.push(
                              `[🟡] Transaction for ${transaction.customerName} canceled!`,
                            );
                          }
                        } catch (e: any) {
                          messages.push(
                            `[🔴] Error canceling transaction: ${e.message}`,
                          );
                        }
                      }
                    }
    ...
    if (product.success && customer.success) {
      ...
      const customerType = (() => {
        const typeNames: string[] = (
          customer.data.customerTypes as object[]
      ...
      try {
        ...
        await sleep(200);
        await page
          .getByPlaceholder("Masukkan 16 digit NIK Pelanggan")
        ...
        if (customer.data.customerTypes.length > 1) {
          ...
          const nextButton = page.getByRole("button", {
            name: "NANTI SAJA, LANJUT TRANSAKSI",
          });
        ...
        for (let x = 0; x < 5; x++) {
          ...
          const puzzleCanvas = await page
            .locator(".rc-slider-captcha-jigsaw-bg")
            .boundingBox();
          const puzzleBg = (
            await page
              .locator(".rc-slider-captcha-jigsaw-bg")
          ...
          const puzzleSlider = await page
            .locator(".rc-slider-captcha-button")
            .boundingBox();
          ...
          if (puzzleSlider) {
            ...
            for (let i = 0; i < puzzleCanvas?.width!; i += step) {
              ...
              const solve = await page
                .locator(".rc-slider-captcha-jigsaw")
                .screenshot();
              const matchPercent = (
                await comparePixelmatchBuffers(puzzleBgBuffer, solve)
              ).percent;
              ...
              if (
                matchLastNum.length == 3 &&
                matchLastNum.every((v) => v == matchPercent)
              ) {
                break;
              }
            ...
            await page.mouse.move(
              puzzleSlider.x + matchLib.indexOf(highestMatch) * step,
              puzzleSlider.y,
            );
            await page.mouse.up();
          }
          ...
          for (let y = 0; y < 50; y++) {
            ...
            if (
              page
                .url()
                .startsWith(
                  "https://subsiditepatlpg.mypertamina.id/merchant/app/sale/struk",
                )
            ) {
              return {
                success: true,
